...
def RLabel(text: Union[str, Signal[str]], font=None, **kwargs) -> WidgetSpec:
    return WidgetSpec(widget_class=_RLabel, props={"text": text, "font": font, **kwargs})
...
def RButton(text: Union[str, Signal[str]], command: Callable = None, **kwargs) -> WidgetSpec:
    return WidgetSpec(widget_class=_RButton, props={"text": text, "command": command, **kwargs})

class _REntry(StatefulComponent):
    def __init__(self, props=None):
        super().__init__(props=props)
        self._tk_var = None
        # Signal registration handled in mount usually, but we can do it here too if needed.
        # But REntry logic for binding is in on_mount
    ...
    def on_mount(self):
        ...
        if signal:
            ...
            def on_tk_change(*args):
                if signal.get() != self._tk_var.get():
                    signal.set(self._tk_var.get())
            
            self._tk_var.trace_add("write", on_tk_change)
            
            # Using weakref subscriber to update Tk var
            def on_sig_change(val):
                if self._tk_var.get() != val:
                    self._tk_var.set(val)
...
def REntry(text_variable: Signal[str] = None, **kwargs) -> WidgetSpec:
    return WidgetSpec(widget_class=_REntry, props={"text_variable": text_variable, **kwargs})
    ...
    def build(self, context: BuildContext) -> WidgetSpec:
        return WidgetSpec(
            ...
            props={
                ...
                "command": self.props.get("command"),
                "text_color": context.theme.get_color("text"),
                **{k:v for k,v in self.props.items() if k not in ["text", "variable", "command"]}
            }
        )

def RCheckbox(text: str, variable: Signal[bool] = None, command=None, **kwargs) -> WidgetSpec:
    return WidgetSpec(widget_class=_RCheckbox, props={"text": text, "variable": variable, "command": command, **kwargs})

class _RDiv(Component):
    # No special init
    
    def build(self, context: BuildContext):
        from rocket.native import NativeColumn
        return WidgetSpec(
...
def RSwitch(checked: bool = False, command=None, **kwargs) -> WidgetSpec:
    return WidgetSpec(widget_class=_RSwitch, props={"checked": checked, "command": command, **kwargs})
